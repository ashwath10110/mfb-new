  ...
  clone(item: any) {
    ...
    this.modal.confirm('Copy', 'Would you like to copy the ' + this.api2 + '?').subscribe(result => {
      if (result) {
        ...
        delete clone._id;
        this.crud.add(this.api2, clone).subscribe(data => { this.flush(); this.filterText = ''; this.getData() }, err => this.snack.open(err, 'OK', { duration: 2000 }));
      }
    })
  }
  delete(item: any) {
    ...
    this.modal.confirm('Delete?', 'Would you like to delete the ' + this.api2 + '?').subscribe(result => {
      if (result) {
        this.crud.delete(this.api2, item._id).subscribe(data => { this.flush(); this.filterText = ''; this.getData() }, err => this.snack.open(err, 'OK', { duration: 2000 }));
      }
    })
  }
